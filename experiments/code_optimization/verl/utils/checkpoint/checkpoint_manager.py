...
def list_ckpt_candidates(path, directory_format="global_step_{}"):
    """On-disk checkpoint dirs as (step, full_path), NEWEST first, by globbing the
    FILESYSTEM (not an in-memory list) so resume across process restarts sees the
    real set. The tracker-referenced step (if present and parseable) is ordered
    first (deduped). Pure function -- no torch/distributed -- so it is independently
    unit-testable and tolerant of a corrupt or empty tracker file."""
    import glob as _glob
    import re as _re
    if path is None:
        return []
    pat = os.path.join(path, directory_format.format("*"))
    found = {}
    for d in _glob.glob(pat):
        if not os.path.isdir(d):
            continue
        m = _re.search(r"global_step_(\d+)$", d)
        if m:
            found[int(m.group(1))] = d
    cands = [(s, found[s]) for s in sorted(found, reverse=True)]
    tracker = get_checkpoint_tracker_filename(path)
    if os.path.exists(tracker):
        try:
            with open(tracker, "rb") as f:
                ti = int(f.read().decode())
            if ti in found:
                cands = [(ti, found[ti])] + [c for c in cands if c[0] != ti]
        except Exception:
            pass
    return cands

...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            # Optional long process-group timeout. A rank0-only hf_model save
            # (full-state gather plus GenerationConfig lookup) holds the other ranks
            # in a collective barrier, and on a large model that can exceed the
            # default 10-minute NCCL watchdog and crash the save. Env-gated; default
            # 0 keeps the torch default, so this is backward-compatible.
            _pg_to = int(os.environ.get("PIE_PG_TIMEOUT_SEC", "0"))
            if _pg_to > 0:
                from datetime import timedelta as _td
                torch.distributed.init_process_group(timeout=_td(seconds=_pg_to))
            else:
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        if self.world_size > 1 and fsdp_version(self.actor.actor_module) == 1:
            if self.actor.actor_module._handle.uses_sharded_strategy:

...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        if self.config.padding_free:
            ...
            if calculate_entropy:  # logged-only; no grad, chunked over the vocab to bound memory
                with torch.no_grad():
                    entropy_rmpad = VF.entropy_from_logits(logits_rmpad)  # (total_nnz / sp + pad,)
                    if self.config.ulysses_size > 1:
                    ...
                    ).squeeze(-1)[:, -response_length - 1 : -1]  # (bsz, response_length)
        else:
            output = self.actor_module(
                input_ids=input_ids,
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    if old_log_probs is None:
                        # skip_old_log_prob: strictly on-policy, so the ratio is identically 1.
                        # .detach() is what makes this exact rather than an approximation --
                        # exp(lp - lp.detach()) evaluates to 1.0 but differentiates to grad lp, so
                        # -A * ratio gives the exact REINFORCE gradient -A * grad log pi. Numerically
                        # this is TIGHTER than the two-pass version, whose ratio is 1 +/- 1e-7
                        # because the two forwards batch the sequences differently.
                    ...
                    # true token entropy (T=1 during training) -- logged ONLY, never in the gradient.
                    batch_metrics["actor/entropy"] = VF.masked_mean(entropy, response_mask).detach().item()
                    append_to_dict(metrics, batch_metrics)

                grad_norm = self._optimizer_step()
                raw_grad_norm = grad_norm.detach().item()
                append_to_dict(
                    metrics,
                    {
                        "actor/grad_norm": raw_grad_norm,  # RAW (pre-clip): clip_grad_norm_ returns the norm it saw
                        # 1.0 when the max_grad_norm guard actually rescaled this update. When
                        # max_grad_norm is set high as a spike guard only, a persistently nonzero
                        # mean here means the guard is silently shaping training, not guarding it.
                        "actor/grad_norm_clipped": float(raw_grad_norm > self.config.max_grad_norm),
                        # a non-finite grad norm SKIPS the optimizer step entirely (_optimizer_step),
                        # which is otherwise invisible in the logs.
                        "actor/grad_norm_nonfinite": float(not math.isfinite(raw_grad_norm)),
                    },
                )

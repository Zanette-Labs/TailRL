...
class FSDPWorker(Worker):
    ...
    def _init_dist_mesh(self, config: Union[ActorConfig, CriticConfig], role: Literal["actor", "critic"]):
        ...
        validate_skip_old_log_prob(config, role)
    ...
    def _build_model_optimizer(
        ...
        if role in ["actor", "critic"]:
            ...
            if optim_config.lr_scheduler_type == "constant":
                ...
                # HF/TRL default schedule -- SE-GUI's protocol (their fork never overrides it, and
                # their released trainer_state shows lr decaying to ~0 over the run). Unlike
                # "constant", this one has a HORIZON: training_steps is baked into the slope, so a
                # resumed slice must recompute the SAME training_steps or the decay jumps. See
                # docs/segui/protocol_resolved.md.
    ...
    def save_checkpoint(self, path: str, save_model_only: bool = False, actor_only_path: Optional[str] = None):
        ...
        self.checkpoint_manager.save_checkpoint(path, save_model_only, actor_only_path=actor_only_path)
        dist.barrier()
        if self._use_param_offload:
            offload_fsdp_model(self.fsdp_module)

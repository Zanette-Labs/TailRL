...
class TrainerConfig:
    ...
    export_actor_only: bool = False
    """errata C2: at every save step ALSO export an actor-weights-only bf16 HF checkpoint (no
    optimizer, unsharded, directly vLLM/HF-loadable) to {save_checkpoint_path}/actor_only/
    global_step_N/actor/. This track lives OUTSIDE the save_limit prune root (the pruner only
    matches global_step_N directly under save_checkpoint_path), so every snapshot is retained."""
    export_actor_only_freq: int = -1
    """export the actor_only snapshot only when global_step % this == 0 (-1 = at every save).
    Decouples the UNPRUNED eval track from a fast full-state save cadence: with save_freq=10
    (paired) an every-save export would retain ~2x16.6 GB per 10 steps -> TBs over a long run;
    export_actor_only_freq=100 keeps the eval ladder at ~11 snapshots. The FINAL save always
    exports regardless, so the end-of-training checkpoint is directly evaluable."""
    extra_save_steps: Tuple[int, ...] = ()
    """explicit global steps that ALSO get a full-state save, on top of the periodic save_freq
    cadence -- and whose actor_only export is FORCED (export_actor_only_freq is bypassed). This is
    what buys a non-uniform eval ladder from one knob: a dense early set (100, 200, 300, 400, ...)
    for the separation-window analysis, plus a sparse periodic cadence for the long tail. Cheap in
    storage: the extra FULL-state saves are still pruned by save_limit, so only the ~8 GB
    actor_only export of each listed step is retained."""
    save_paired: bool = False
    """save a full-state checkpoint at BOTH global_step % save_freq == save_freq - 1 AND == 0
    (adjacent steps, e.g. 9,10 / 19,20 with save_freq=10). With save_limit=2 the two retained
    checkpoints are 1 step apart, so a corrupt/unloadable newest checkpoint costs 1 step of
    progress on load-fallback (find_complete_ckpts + RayPPOTrainer._load_checkpoint), not
    save_freq steps."""
...
class PPOConfig:
    ...
    def post_init(self):
        ...
        # The actor writes its OWN progress bars, which rollout.disable_tqdm never covered. Ungated
        # they emit ~2 GB/hour PER ARM: 470 GB accumulated in 1.5 days, filled the checkpoint pool,
        # and killed all three 3B arms mid-save (PytorchStreamWriter ... unexpected pos 1). One flag
        # should silence every progress bar in the run, so it is propagated here.
        self.worker.actor.disable_tqdm = self.worker.rollout.disable_tqdm

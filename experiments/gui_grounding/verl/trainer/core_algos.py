...
@register_adv_estimator(AdvantageEstimator.TAILRL)
def compute_tailrl_outcome_advantage(
    ...
    **kwargs,
    ...
    """TailRL (gap-over-survivors) advantage.

    NORMALIZATION, and how it differs from the canonical TailRL in this repository.
    The reference implementation (experiments/text_maze/verl/trainer/ppo/core_algos.py and
    experiments/imagenet_localization/core/advantages.py) multiplies the survival integral by N
    before mean-centering; this one does not. So the advantages produced here are exactly
    canonical_TailRL / N -- verified numerically to float round-off, and exact algebraically
    because scaling commutes with mean-centering: (c*w) - mean(c*w) = c*(w - mean(w)).

    N is fixed at 8 for every group in this experiment (worker.rollout.n), so the difference is a
    single global constant on the policy-gradient term, not a per-group reweighting: the ordering
    and relative magnitudes within every group are identical. Under AdamW the constant is largely
    absorbed, since m/sqrt(v) is invariant to a global gradient scale.

    This file is kept as it was RUN, because it is what produced paper_results/. Do not "fix" it to
    match the reference without re-running: an 8x change in the effective advantage scale can still
    change when max_grad_norm clipping engages.

    Centered survival-integral advantage of the ordinal cross-entropy objective
    J_ord(x) = ∫_0^1 log p_τ(x) dτ, with p_τ = P(r > τ | x)  (OrdCE proof).

    For a rollout with scalar reward r in a group of N rollouts (Algorithm 1):
        w(r) = ∫_0^r dτ / S(τ) ,   S(τ) = Σ_j 1{r_j > τ}   (integer survivor count)
    computed in closed form via sorted-reward gaps, then MEAN-CENTERED within the group:
        w_(i) = w_(i-1) + (r_(i) - r_(i-1)) / (N - i + 1) ,   A_(i) = w_(i) - mean(w).
    The per-gap weight 1/(N - i + 1) runs from 1/N (smallest reward, N survivors) up to 1
    (largest reward, 1 survivor). No std normalization — dividing by the group std
    reintroduces the coefficient-estimation overhead that the constant-weight objective
    eliminates (Thm 5.1). Binary rewards reduce this exactly to centered MaxRL: each of the
    K top-tied winners gets 1/K - 1/N, every loser gets -1/N  (Prop 6.1).
    """
    scores = token_level_rewards.sum(dim=-1)  # (bs,) scalar outcome reward per rollout
    id2score = defaultdict(list)
    ...
    for idx in id2score:
        group = torch.stack(id2score[idx]).to(torch.float64)  # (N,) group rewards
        n = group.numel()
        if n == 1:  # singleton group: no relative signal, zero advantage
            advantages[id2pos[idx][0]] = 0.0
            continue
        order = torch.argsort(group)                          # ascending r_(1)..r_(n)
        r_sorted = group[order]
        prev = torch.cat([r_sorted.new_zeros(1), r_sorted[:-1]])
        gaps = r_sorted - prev                                # gap_m = r_(m) - r_(m-1), r_(0)=0
        survivors = torch.arange(n, 0, -1, dtype=torch.float64, device=group.device)  # n..1
        inv_survival = 1.0 / survivors                        # 1 / S(τ) on each interval, in [1/n, 1]
        a_sorted = torch.cumsum(gaps * inv_survival, dim=0)   # w_(k) = ∫_0^{r_(k)} dτ / S(τ)
        a_group = torch.empty_like(a_sorted)
        a_group[order] = a_sorted                             # back to within-group order
        a_group = a_group - a_group.mean()                    # mean-center (baseline)
        for j, i in enumerate(id2pos[idx]):
            advantages[i] = a_group[j].to(scores.dtype)
...
def compute_policy_loss(
    ...
    tau_positive: float,
    tau_negative: float,
    loss_type: Literal["default", "vanilla", "gspo", "gspo_token", "cispo", "sapo"],
    ...
    **kwargs,
    ...
    if loss_type == "vanilla":  # pure policy gradient, NO clipping. With ppo_epochs=1 the policy
        # is on-policy (ratio ~ 1), so this is the exact REINFORCE/PG gradient of -A * logprob.
        pg_loss = average_loss(-advantages * ratio, response_mask, mode=loss_avg_mode)
        return pg_loss, {
            "pg_clipfrac_higher": 0.0,
            "pg_clipfrac_lower": 0.0,
            "ppo_kl": VF.masked_mean(-negative_approx_kl, response_mask).detach().item(),
            # real policy entropy (E_{a~pi}[-log pi(a)] == H(pi) per token on-policy at ppo_epochs=1):
            # logged only (no entropy bonus, see config) so the exploration curve is visible, not flat.
            "entropy_loss": average_loss(-log_probs, response_mask, mode=loss_avg_mode).detach().item(),
        }

...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, path: Optional[str] = None):
        ...
        # The optimizer shard can be corrupt if a save was interrupted mid-write (zip "central directory"
        # error). Fall back to a model-only load with a FRESH optimizer rather than crashing -- the policy
        # weights are what matter; Adam state rewarms quickly at constant LR.
        try:
            ...
            print(f"[rank-{self.rank}]: optimizer checkpoint unreadable ({e}); loading model only with a FRESH optimizer.")
            set_model_state_dict(self.model, model_state_dict, options=state_dict_options)
        # extra_state (lr_scheduler + RNG) can be absent if a checkpoint save was interrupted mid-write.
        # Tolerate that so a resume can proceed from model+optim alone (scheduler/RNG reset is harmless
        # at constant LR) instead of crashing or silently falling back to scratch.
        if os.path.exists(extra_path):
            ...
            print(f"[rank-{self.rank}]: extra_state missing ({extra_path}); resuming model+optim only.")

    def save_checkpoint(self, path: str, save_model_only: bool = False, actor_only_path: Optional[str] = None):
        ...
        # errata C2: also emit an actor-weights-only, unsharded bf16 HF checkpoint (no optimizer,
        # no per-rank shards) that vLLM/transformers can load directly -- no offline merge step.
        if actor_only_path is not None:
            self._export_actor_only(actor_only_path)
            dist.barrier()

    def _export_actor_only(self, actor_only_path: str) -> None:
        """Gather the FULL (unsharded) policy weights onto rank-0 CPU and write a directly
        HF/vLLM-loadable bf16 checkpoint to `actor_only_path` (errata C2). Non-zero ranks receive an
        empty state dict from the gather and write nothing. LoRA is not supported here (the headline
        run is full fine-tune); for a PeftModel this exports the wrapped base+adapter as merged HF
        weights only if already merged, so we guard on PreTrainedModel."""
        # full_state_dict=True gathers the complete dict to rank-0 (cpu_offload keeps it off-GPU);
        # ~16.6 GB bf16 for the 7B policy, well within rank-0's CPU budget.
        full_state_dict = get_model_state_dict(
            self.model, options=StateDictOptions(full_state_dict=True, cpu_offload=True)
        )
        if self.rank == 0:
            hf_module = self.model._fsdp_wrapped_module
            assert isinstance(hf_module, PreTrainedModel), (
                "export_actor_only requires a PreTrainedModel policy (full fine-tune)"
            )
            # guarantee bf16 on disk regardless of the FSDP param dtype (no-op if already bf16)
            full_state_dict = {
                k: (v.to(torch.bfloat16) if v.is_floating_point() and v.dtype != torch.bfloat16 else v)
                for k, v in full_state_dict.items()
            }
            os.makedirs(actor_only_path, exist_ok=True)
            print(f"[rank-{self.rank}]: Exporting actor-only HF checkpoint to {os.path.abspath(actor_only_path)}.")
            hf_module.save_pretrained(actor_only_path, state_dict=full_state_dict, safe_serialization=True)
            self.processing_class.save_pretrained(actor_only_path)
        del full_state_dict

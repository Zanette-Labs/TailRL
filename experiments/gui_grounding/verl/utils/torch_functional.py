...
def entropy_from_logits(logits: torch.Tensor, chunk_size: int = 512) -> torch.Tensor:
    """Token-level Shannon entropy H = -sum_v softmax(logits)_v * log_softmax(logits)_v.

    Computed in fp32 and chunked over the flattened token dimension so the full per-token vocab
    distribution is materialised only `chunk_size` rows at a time (bounds peak memory on large
    vocabularies). Returns logits[..., V] -> [...]. For LOGGING only (call under torch.no_grad()).

    Args:
        logits (torch.Tensor): shape (..., vocab_size)
    Returns:
        torch.Tensor: entropy per position, shape (...)
    """
    lead = logits.shape[:-1]
    flat = logits.reshape(-1, logits.shape[-1])
    out = torch.empty(flat.shape[0], dtype=torch.float32, device=logits.device)
    for i in range(0, flat.shape[0], chunk_size):
        logp = torch.log_softmax(flat[i : i + chunk_size].float(), dim=-1)
        out[i : i + chunk_size] = -(logp.exp() * logp).sum(dim=-1)
    return out.view(*lead)
...
def get_linear_schedule_with_warmup(
    ...
    """Warmup then LINEAR decay to 0 at num_training_steps.

    The lambda is HuggingFace's `get_linear_schedule_with_warmup` verbatim (transformers
    optimization.py). That is not an arbitrary choice: it is the HF/TRL DEFAULT
    lr_scheduler_type, which SE-GUI's fork inherits without overriding, so reproducing their
    protocol means reproducing this exact decay. tests/segui/test_linear_scheduler.py asserts
    the parity against transformers' own implementation step by step.
    """

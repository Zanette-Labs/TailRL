...
from verl.utils.reward_score.maze import (
    compute_advantage_diagnostics as _maze_advantage_diagnostics,
    compute_maze_aggregate_metrics as _maze_aggregate_metrics,
)

# `process_validation_metrics` does an O(prompts x fields x subset_sizes x
# bootstrap_samples) max/min bootstrap.  Only 0/1 fields produce meaningful
# Pass@k / best@N curves, so we restrict bootstrap to a small whitelist plus
# the always-present `reward` scalar.  Continuous diagnostic fields (R_dist,
# R_sol, path_length, length_ratio, ...) are aggregated cheaply via
# `compute_maze_aggregate_metrics` instead.
_MAZE_BOOTSTRAP_FIELDS = frozenset({
    "reward",
    "goal_reached",
    "wall_collision",
    "valid_format",
    ...
    "is_partial_success",
    "exceeded_UB",
    # Composite reward components (R_dist + R_sol = score). Both are floats
    # in [0, 0.5]; bootstrap mean@N / best@N / worst@N are all meaningful.
    "R_dist",
    "R_sol",
})
...
def compute_advantage(
    ...
    # Optional per-group reward transform applied before ANY advantage estimator.
    # "cdf" replaces each outcome reward by its within-group empirical CDF value
    # (min -> 0, max -> 1, average ranks for ties); token_level_scores and the
    # reward metrics computed from it stay raw.
    reward_transform = config.get("reward_transform", "raw") if config is not None else "raw"
    if reward_transform in (None, "raw"):
        pass
    elif reward_transform == "cdf":
        if "uid" not in data.non_tensor_batch:
            raise ValueError("algorithm.reward_transform=cdf requires uid-grouped rollouts")
        data.batch["token_level_rewards"] = core_algos.compute_cdf_transformed_rewards(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=data.batch["response_mask"],
            index=data.non_tensor_batch["uid"],
        )
    else:
        raise ValueError(f"Unknown algorithm.reward_transform: {reward_transform} (expected raw|cdf)")
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.PKPO_CONTINUOUS,
    ...
    def _validate(self):
        ...
        # Filter heavy / sentinel-laden fields out of the bootstrap pipeline.
        # Maze data sources only need bootstrap on the binary outcome flags;
        # continuous fields are picked up by compute_maze_aggregate_metrics below.
        if any(str(ds).startswith("maze") for ds in set(data_sources.tolist())):
            bootstrap_extras = {
                k: v
                for k, v in reward_extra_infos_dict.items()
                if k in _MAZE_BOOTSTRAP_FIELDS
            }
        else:
            bootstrap_extras = reward_extra_infos_dict
        data_src2var2metric2val = process_validation_metrics(data_sources, sample_inputs, bootstrap_extras)
        ...
        # -------- maze-specific aggregate metrics (per data source) --------
        try:
            maze_sources = [ds for ds in set(data_sources.tolist()) if str(ds).startswith("maze")]
            for ds in maze_sources:
                mask = data_sources == ds
                sub_extra = {
                    k: [v[i] for i in np.where(mask)[0]]
                    for k, v in reward_extra_infos_dict.items()
                    if isinstance(v, list) and len(v) == len(data_sources)
                }
                agg = _maze_aggregate_metrics(sub_extra, data_source=str(ds))
                for k, v in agg.items():
                    metric_dict[f"val/{ds}/{k}"] = v
        except Exception as agg_err:
            print(f"[maze_aggregate_metrics] val skipped: {agg_err}")
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if self.config.trainer.get("val_only", False):
                # flush before exiting: wandb only commits the last history row on
                # finish(), so a bare `return` here loses this validation entirely.
                logger.finish()
                return
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        try:
                            adv_index = batch.non_tensor_batch.get("uid", None)
                            adv_metrics = _maze_advantage_diagnostics(
                                batch.batch["advantages"],
                                batch.batch["response_mask"],
                                index=adv_index,
                            )
                            metrics.update(adv_metrics)
                        except Exception as adv_diag_err:
                            print(f"[advantage_diagnostics] skipped: {adv_diag_err}")

                        try:
                            ds_arr = batch.non_tensor_batch.get("data_source", None)
                            if ds_arr is not None and any(str(d).startswith("maze") for d in ds_arr):
                                ds_unique = np.unique(ds_arr)
                                for ds_val in ds_unique:
                                    if not str(ds_val).startswith("maze"):
                                        continue
                                    mask = ds_arr == ds_val
                                    sub_extra = {
                                        k: [v[i] for i in np.where(mask)[0]]
                                        for k, v in reward_extra_infos_dict.items()
                                        if isinstance(v, (list, tuple)) and len(v) == len(ds_arr)
                                    }
                                    sub_extra["score"] = batch.batch["token_level_scores"].sum(-1)[mask].cpu().tolist()
                                    agg = _maze_aggregate_metrics(sub_extra, data_source=str(ds_val))
                                    for k, v in agg.items():
                                        metrics[f"train/{ds_val}/{k}"] = v
                        except Exception as agg_err:
                            print(f"[maze_aggregate_metrics] train skipped: {agg_err}")
                ...
                if is_last_step:
                    ...
                    progress_bar.close()
                    # flush before exiting: wandb only commits the last history row on
                    # finish(). Without this the end-of-training validation (the number
                    # the whole run exists to produce) is silently lost -- it is the one
                    # log with no later step to force its commit.
                    logger.finish()
                    return

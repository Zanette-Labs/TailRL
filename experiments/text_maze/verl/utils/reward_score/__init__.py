...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    if data_source.startswith("maze"):
        from .maze import judge_maze, judge_maze_binary_shortest, judge_maze_continuous, judge_maze_composite
        if data_source == "maze_17_binary_shortest":
            res = judge_maze_binary_shortest(solution_str=solution_str, ground_truth=ground_truth)
        elif data_source == "maze_17_continuous":
            res = judge_maze_continuous(solution_str=solution_str, ground_truth=ground_truth)
        elif data_source == "maze_17_composite":
            res = judge_maze_composite(solution_str=solution_str, ground_truth=ground_truth)
        else:

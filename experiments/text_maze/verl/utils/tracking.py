...
class Tracking:
    ...
    def finish(self):
        """Flush and close every backend. Safe to call more than once.

        This MUST be called explicitly at the end of training. wandb only *commits* a
        history row once a later step is logged or finish() runs, so the final logged
        step -- which is the end-of-training validation -- is silently dropped if the
        process just exits. Relying on __del__ alone is not enough: CPython does not
        guarantee finalizers run at interpreter shutdown, and they never run when the
        job is SIGKILLed (e.g. SLURM preemption). This is what made the step-5k
        validation missing from ~half the runs of the v3 maze campaign.
        """
        if getattr(self, "_finished", False):
            return
        self._finished = True
        for name, closer in (
            ("wandb", lambda lg: lg.finish(exit_code=0)),
            ("swanlab", lambda lg: lg.finish()),
            ("vemlp_wandb", lambda lg: lg.finish(exit_code=0)),
            ("tensorboard", lambda lg: lg.finish()),
            ("clearnml", lambda lg: lg.finish()),
        ):
            if name in self.logger:
                try:
                    closer(self.logger[name])
                except Exception as e:  # never let teardown kill a finished run
                    print(f"[Tracking.finish] {name} finish failed: {type(e).__name__}: {e}")

    def __del__(self):
        # Best-effort fallback only; the explicit finish() in the trainer is what counts.
        try:
            self.finish()
        except Exception:
            pass

...
class HFRollout(BaseRollout):
    ...
    def _generate_minibatch(self, prompts: DataProto) -> DataProto:
        ...
        # Optionally also terminate generation at extra token ids (e.g. a task-specific
        # DONE token) so generation stops there instead of running to max_new_tokens and
        # emitting reward-invisible junk that destabilises RL. Opt-in via the rollout
        # config key `extra_eos_token_ids`; when unset this is a no-op, so every other
        # experiment using HFRollout is unaffected. Both model.generate (stop condition)
        # and get_response_mask (response mask) accept a list of eos ids, and this single
        # local variable feeds both, so the terminator token is kept while everything
        # after it is masked.
        _extra_eos = self.config.get("extra_eos_token_ids", None)
        if _extra_eos:
            _base_eos = list(eos_token_id) if isinstance(eos_token_id, (list, tuple)) else [eos_token_id]
            eos_token_id = _base_eos + [int(i) for i in _extra_eos if int(i) not in _base_eos]
